...
def getstream():  # put application's code here
    ...
    if param == "getts":
        source = request.url
        source = source.replace('https://workgeme.herokuapp.com/getstream?param=getts&source=','')
    ...
    if param == "getm3u8":
        ...
        if "FullscreenAllowed" in r.text:
            ...
            if "m3u8" in veri:
                ...
                '''tsal = tsal.replace('https://workgeme.herokuapp.com/getstream?param=getts&source=https://')'''
                return tsal
        else:
            return "Veri yok"
